<?php
  ...
  //GET ASIGNATURA ID
  $asignaturaResult = $conn->query("SELECT id_asignatura FROM evaluaciones WHERE id = $idEvaluacion");
  while ($row = $asignaturaResult->fetch_array(MYSQLI_ASSOC)) {
      $idAsignatura = $row["id_asignatura"];
  }

  //GET PORCENTAJE DE LA EVALUACION
  $porcEvaluacionResult = $conn->query("SELECT porcentaje FROM evaluaciones WHERE id = $idEvaluacion");
  while ($row = $porcEvaluacionResult->fetch_array(MYSQLI_ASSOC)) {
      $porcentajeEvaluacion = $row["porcentaje"];
  }
    ...
    while ($row = $totalPorcResult->fetch_array(MYSQLI_ASSOC)) {
        ...
        $porcentajeEvaluaciones = $row["porcentajeEvaluaciones"] - $porcentajeEvaluacion;
    }
